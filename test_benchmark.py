...
def plot_policy(policy,M,O):
    policy_np =  np.zeros(M*O)
    for i in range(M):
        for j in range(O):
            state = j*M + i
            print(state)

            policy_np[state] = policy(i,j,0)
    plt.figure()
    plt.plot(np.arange(M*O),policy_np)
...
policy_parameters_load[:,:,0::3] = np.array([[M-10,M-5,M],[M-20,M-15,M-10],[M-30,M-25,M-20],[M-40,M-35,M-30],[M-50,M-45,M-40]])
policy_parameters_load[:,:,1::3] = np.array([[M,M,M],[M-10,M-5,M],[M-20,M-15,M-10],[M-30,M-25,M-20],[M-40,M-35,M-30]])
    ...
    for mc_round in tqdm.tqdm(range(N_mc)):
        ...
        for policy in range(len(policies)):
            ...
            for round_ in np.arange(N_rounds-1):
                ...
                class_counts[policy,mc_round,round_+1,current_learner,:] = oracle.class_dist[oracle.learner_class_preference[current_learner]]
...
plt.figure()
fig,axs = plt.subplots(1,N_Learners)
for i in range(N_Learners):
    axs[i].plot(np.arange(N_rounds),oracle_states_store.mean(0)[0,:,i],label="Learner "+str(i))
plt.legend()
plt.savefig("figures/oracle_states.png")