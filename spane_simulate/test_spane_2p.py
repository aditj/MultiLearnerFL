...
fs = np.array([[0,0.2,0.4,0.6,0.9],[0,0.2,0.4,0.5,0.9]])
                ...
                for oracle_state_2 in range(O):
                    ...
                    for learner_state_1 in range(L):
                        for learner_state_2 in range(L):
                            ...
                            min_action = 0
                            for action in range(U):
                                cost_incurred = retrieve_cost(current_learner,current_learner,current_learner_state,current_oracle_state,action,C_L,C_Q_L,C_Q_O,lagrange[current_learner],N=2,cost_type='zero_sum')
                            # print(cost_incurred)
                            ...
                            for learner_index in range(N):
                                if learner_index==current_learner:
                                    ...
                                    cost_incurred = retrieve_cost(learner_index,current_learner,current_learner_state,current_oracle_state,min_action,C_L,C_Q_L,C_Q_O,lagrange[learner_index],N=2,cost_type='zero_sum')
...
from utils import get_action_from
n_mc = 1
n_iter = 1000000
...
if DO_SPANE:
    for mc in tqdm.tqdm(range(n_mc)):
        ...
        scale_parameter = 200
        step_parameters = np.linspace(2,0.1,n_iter)
        learner_states = np.ones((N),dtype=int)*(L-1)
        oracle_states = np.array([1,4])
        C_Q_L_zeros = np.zeros((L,U))
        C_Q_O_zeros = np.zeros((N,O))
        C_L_zeros = np.zeros((N,O,U))
        delta_value = [0.5,0.5,np.pi/8]
        delta_value = np.vstack([delta_value]*O)
        for i in range(n_iter):
            step_parameter = step_parameters[i]
            current_learner = get_current_learner(learner_states,oracle_states)
            current_learner_state = learner_states[current_learner]
            current_oracle_state = oracle_states[current_learner]
            ...
            learning_cost_incurred_plus = retrieve_cost(current_learner,current_learner,current_learner_state,current_oracle_state,current_action,C_L,C_Q_L_zeros,C_Q_O_zeros,lagrange_parameters[current_learner],N=2,cost_type='zero_sum')
            queue_cost_incurred_plus = retrieve_cost(current_learner,current_learner,current_learner_state,current_oracle_state,current_action,C_L_zeros,C_Q_L,C_Q_O,lagrange_parameters[current_learner],N=2,cost_type='zero_sum')
            current_action = get_action_from(policy_parameters_minus[current_oracle_state],current_learner_state)
            learning_cost_incurred_minus = retrieve_cost(current_learner,current_learner,current_learner_state,current_oracle_state,current_action,C_L,C_Q_L_zeros,C_Q_O_zeros,lagrange_parameters[current_learner],N=2,cost_type='zero_sum')
            queue_cost_incurred_minus = retrieve_cost(current_learner,current_learner,current_learner_state,current_oracle_state,current_action,C_L_zeros,C_Q_L,C_Q_O,lagrange_parameters[current_learner],N=2,cost_type='zero_sum')
            current_action = get_action_from(policy_parameters_current[current_oracle_state],current_learner_state)
            queuecost_current = retrieve_cost(current_learner,current_learner,current_learner_state,current_oracle_state,current_action,C_L_zeros,C_Q_L,C_Q_O,lagrange_parameters[current_learner],N=2,cost_type='zero_sum')
            if np.isnan(learning_cost_incurred_plus) or np.isnan(learning_cost_incurred_minus) or np.isnan(queue_cost_incurred_plus) or np.isnan(queue_cost_incurred_minus) or np.isnan(queuecost_current):
                print("Error",learning_cost_incurred_plus,learning_cost_incurred_minus,queue_cost_incurred_plus,queue_cost_incurred_minus,queuecost_current)
                raise Exception
            ...
            policy_parameters[:,:,0] = np.clip(policy_parameters[:,:,0],-100,100)
            policy_parameters[:,:,1] = np.clip(policy_parameters[:,:,1],-100,100)
...
true_estimates = np.array([[20,19,14,11,6],[20,19,18,11,6]])
for o in range(O):
    axs[0,o].set_title('Oracle State {}'.format(o+1))

    sns.lineplot(policy_parameters_store.mean(axis=0)[:,0,o,0],ax=axs[0,o],label='Estimate $\\theta$')
    axs[0,o].scatter(x=n_iter,y=true_estimates[0,o],label='True Value $\phi$', )
    sns.lineplot(policy_parameters_store.mean(axis=0)[:,1,o,0],ax=axs[1,o],label='Estimate $\\theta$')
    ax=axs[1,o].scatter(x=n_iter,y=true_estimates[1,o],label='True Value $\phi$')
### set labels
axs[0,0].set_ylabel('Learner 1 Parameters',fontsize=18)
axs[1,0].set_ylabel('Learner 2 Parameters',fontsize=18)
plt.legend()


plt.savefig('policy_parameters.eps')
